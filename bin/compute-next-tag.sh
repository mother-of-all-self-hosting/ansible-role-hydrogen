...
#
# Tags look like `v<Hydrogen version>-<release>`:
#
# - if defaults/main.yml points at a Hydrogen version that has never been
#   released, the release counter restarts at 0 (`v0.5.1-0`)
...
#
# It also keeps releases flowing while Hydrogen itself is quiet: upstream has
# not cut a release since v0.5.1, so a scheme keyed on Renovate's version bumps
# would have nothing to tag no matter how much the role changed.
...
version="$(sed -nE 's|^hydrogen_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Hydrogen version from $defaults_path"
	exit 1
fi

# Hydrogen's version values carry a leading `v` (e.g. `v0.5.1`) and so do the
# tags (`v0.5.1-0`). Stripping any `v` before prepending one keeps this
